#!/usr/bin/env python
"""
Bot that uses propositional logic in phase 1 to determine optimal moves according to a predetermined strategy.
Plays randomly when leading tricks in phase 1.
...
class Bot:
    ...
    def standard_move(self, state, moves, trump_moves, other_moves):
    ...
    def get_move(self, state):
        ...
        return self.standard_move(state, moves, trump_moves, other_moves)
        
    def kb_consistent(self, state, move, strategy):  # checks KB if move is part of strategy
        kb = KB()

        # load strategy file
        path = f"bots.stratbotr.load_{strategy}"
        try:
